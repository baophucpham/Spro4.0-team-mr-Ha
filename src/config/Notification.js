...
import { getFirstNotiItemAction } from 'action/authenAction'
import { getListRoomMessage } from 'action/messageAction'
import { Screens } from 'common'
import { NotiType } from 'common/data'
...
const getDeviceToken = async () => {
	try {
		...
		LogUtil.e(token, 'TOKEN FIREBASE:')
		return token
	} catch (error) {
		return ''
	}
...
const handleNotiServer = (notification, callback) => {
	const { type = '' } = notification?.data
	callback?.(notification)
	switch (type) {
		case NotiType.LIKE:
		case NotiType.TAG_POST:
		case NotiType.COMMENT:
		case NotiType.ADD_FRIEND:
		case NotiType.ACCEPT_FRIEND:
			store.dispatch(getFirstNotiItemAction())
			break
		case NotiType.CHAT:
			store.dispatch(getListRoomMessage(1))
			break
		default:
			break
	}
...
const handleTouch = (notification) => {
	const { type = '', post_id = '', room_id } = notification?.data
	switch (type) {
		case NotiType.LIKE:
		case NotiType.TAG_POST:
		case NotiType.COMMENT:
			Navigator.navigate(Screens.Comment, { post_id: post_id })
			break
		case NotiType.CHAT:
			Navigator.navigate(Screens.Message, { room: { id: room_id } })
			break
		default:
			break
	}
...
const handleNotify = async (callback) => {
	...
	PushNotification.configure({
		...
		onNotification: function (notification) {
			handleNotiServer(notification, callback)
			if (Platform.OS === 'ios') {
